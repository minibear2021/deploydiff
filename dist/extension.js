...
var DiffDirectionIndicator = class {
  statusBarItem = vscode13.window.createStatusBarItem(vscode13.StatusBarAlignment.Left, 99);
  disposables = [];
  constructor() {
    this.statusBarItem.name = "DeployDiff Direction";
    this.disposables.push(
      vscode13.window.tabGroups.onDidChangeTabs(() => this.update()),
      vscode13.window.onDidChangeActiveTextEditor(() => this.update())
    );
    this.update();
  }
  update() {
...
Revert Block pushes right \u2192 left (${rightRole} \u2192 ${leftRole})`;
  ...
  getActiveDiffInput() {
    const activeTab = vscode13.window.tabGroups.activeTabGroup.activeTab;
    if (!activeTab || !(activeTab.input instanceof vscode13.TabInputTextDiff)) {
      return void 0;
    }
