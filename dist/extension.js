...
var RemoteDiffDocumentProvider = class {
  ...
  watch() {
    return new vscode5.Disposable(() => void 0);
  }
  async stat(uri) {
  ...
  async writeFile(uri, content) {
    ...
    this.didChangeFileEmitter.fire([{ type: vscode5.FileChangeType.Changed, uri }]);
  }
  readDirectory() {
    return [];
  }
  createDirectory() {
    throw vscode5.FileSystemError.NoPermissions("DeployDiff remote documents do not support directory creation here.");
  }
  delete() {
    throw vscode5.FileSystemError.NoPermissions("DeployDiff remote documents do not support delete from the editor.");
  }
  rename() {
    throw vscode5.FileSystemError.NoPermissions("DeployDiff remote documents do not support rename from the editor.");
  }
...
function getOrResolveResourceUri(resource) {
  ...
  const activeUri = vscode6.window.activeTextEditor?.document.uri;
...
function getDeploymentMappings(workspaceFolder) {
  const configuration = vscode6.workspace.getConfiguration("deploydiff", workspaceFolder.uri);
...
function resolveDeploymentTarget(localFileUri) {
  const workspaceFolder = vscode6.workspace.getWorkspaceFolder(localFileUri);
  if (!workspaceFolder) {
...
async function openDeployedDiff(localFileUri, remoteDiffDocumentProvider) {
  ...
  if (remoteDocument.languageId !== localDocument.languageId) {
    await vscode7.languages.setTextDocumentLanguage(remoteDocument, localDocument.languageId);
  }
